...
DIF = zeros(length(data_1min),1);
DEA = zeros(length(data_1min),1);
MACDValue = zeros(length(data_1min),1);
...
[DIF,DEA,MACDValue]=MACD(Close_1min,FastLength,SlowLength,DEALength);
...
QuitPrice = 0;
ATRValue = ATR(High_1min,Low_1min,Close_1min,LongLen,0);
Lots = 0;
%% --策略仿真--
...
for i=LongLen+2:length(data)
    ...
    while (isSameDay ==1) && (CurrentMinBarIndex~=length(Date_1min)) 
        ...
        if MarketPosition==-1
            if DIF(CurrentMinBarIndex)>=0 %|| Close_1min(CurrentMinBarIndex) > QuitPrice
                MarketPosition=0;
                ShortMargin(CurrentMinBarIndex)=0;     %平空后空头保证金为0了
                
                %ThisQuitPrice = QuitPrice;
                %if Close_1min(CurrentMinBarIndex) + Slip*MinMove*PriceScale < QuitPrice
                    ThisQuitPrice = Close_1min(CurrentMinBarIndex) - Slip*MinMove*PriceScale;
                %end
                
                for j=1:Lots
                    ClosePosNum=ClosePosNum+1;
                    ClosePosPrice(ClosePosNum)= ThisQuitPrice;%记录平仓价格
                ...
                if  DIF(CurrentMinBarIndex)>=0
                    MarketPosition = 1;
                    MyEntryPrice(CurrentMinBarIndex) = Close_1min(CurrentMinBarIndex)  - Slip*MinMove*PriceScale; 
                ...
                %if QuitPrice > Low_1min(CurrentMinBarIndex) + round(5*ATRValue(i))
                %    QuitPrice = Low_1min(CurrentMinBarIndex) + round(5*ATRValue(i));
                %end
            end
       
        elseif MarketPosition==0 
            if DIF(CurrentMinBarIndex - 1) >= 0 && DIF(CurrentMinBarIndex - 2) <0
                %Open Long
                MarketPosition = 1;
                ...
                OpenDate(OpenPosNum)=Date_1min(CurrentMinBarIndex);%记录开仓时间
                Type(OpenPosNum)=1;   %方向为多头
                %QuitPrice = OpenPosPrice(OpenPosNum) - round(5*ATRValue(i));
                Lots = 1;
                StaticEquity(CurrentMinBarIndex)=StaticEquity(CurrentMinBarIndex-1);
            ...
            elseif DIF(CurrentMinBarIndex - 1) <= 0 && DIF(CurrentMinBarIndex - 2) > 0
                %Open Short
                MarketPosition = -1;
                MyEntryPrice(CurrentMinBarIndex)= Open_1min(CurrentMinBarIndex) - Slip*MinMove*PriceScale;               
                OpenPosNum=OpenPosNum+1;
        ...
        elseif MarketPosition==1
            ...
            if DIF(CurrentMinBarIndex) <= 0 %|| Close_1min(CurrentMinBarIndex) < QuitPrice
                ...
                for j=1:Lots
                    ClosePosNum=ClosePosNum+1;
                    ClosePosPrice(ClosePosNum)=ThisQuitPrice;
                    CloseDate(ClosePosNum)=Date_1min(CurrentMinBarIndex);%记录平仓时间
                end
                EarnPoint = 0;
                for j=1:Lots
                ...
                if DIF(CurrentMinBarIndex) <= 0
                    MarketPosition = -1;
                    MyEntryPrice(CurrentMinBarIndex) = OpenPosPrice(OpenPosNum) + Slip*MinMove*PriceScale; 
                    OpenPosNum=OpenPosNum+1;
                    OpenPosPrice(OpenPosNum)=MyEntryPrice(CurrentMinBarIndex);%记录开仓价格
                    OpenDate(OpenPosNum)=Date_1min(CurrentMinBarIndex);%记录开仓时间
                    Type(OpenPosNum)=1;   %方向为多头
                ...
                %if QuitPrice < High_1min(CurrentMinBarIndex) - round(5*ATRValue(i))
                %    QuitPrice = High_1min(CurrentMinBarIndex) - round(5*ATRValue(i));
                %end
            end
