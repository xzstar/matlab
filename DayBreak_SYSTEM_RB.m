...
Date=datenum(num2str(data(:,1)),'yyyymmdd');                %日期时间
Open=data(:,2);               %开盘价
High=data(:,3);               %最高价
Low=data(:,4);                %最低价
Close=data(:,5);              %收盘价
...
for i=LongLen:length(data)
    ...
    %开多头
    if MarketPosition~=1 && Close(i)> MALong(i)
...
DailyEquity=DynamicEquity;%DynamicEquity(hour(Date)==9  & minute(Date)==0 & second(Date)==0);
...
TradeSum{RowNum,7} = 0; %mean(DailyRet)*365*100;

%年度收益率(按周算)
RowNum = 12;
ColNum = 1;
TradeSum{RowNum,6} = '年度收益率(按周算,%)';
TradeSum{RowNum,7} = 0; %mean(WeeklyRet)*52*100;

%年度收益率(按月算)
RowNum = 13;
ColNum = 1;
TradeSum{RowNum,6} = '年度收益率(按月算,%)';
TradeSum{RowNum,7} = 0;%mean(MonthlyRet)*12*100;

%夏普比率(按日算)
RowNum = 14;
ColNum = 1;
TradeSum{RowNum,6} = '夏普比率(按日算,%)';
TradeSum{RowNum,7} = 0;%(mean(DailyRet)*365-RiskLess)/(std(DailyRet)*sqrt(365));

%夏普比率(按周算)
RowNum = 15;
ColNum = 1;
TradeSum{RowNum,6} = '夏普比率(按周算,%)';
TradeSum{RowNum,7} = 0;%(mean(WeeklyRet)*52-RiskLess)/(std(WeeklyRet)*sqrt(52));

%夏普比率(按月算)
RowNum = 16;
ColNum = 1;
TradeSum{RowNum,6} = '夏普比率(按月算,%)';
TradeSum{RowNum,7} = 0;%(mean(MonthlyRet)*12-RiskLess)/(std(MonthlyRet)*sqrt(12));
